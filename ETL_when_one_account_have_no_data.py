 #if all account have not data so some account works in it

!pip install facebook-business pyarrow aiohttp google-cloud-bigquery
!pip install shapely --upgrade -q
...
FACEBOOK_APP_ID       = "127868"
FACEBOOK_APP_SECRET   = "486da8c56"
FACEBOOK_ACCESS_TOKEN = "EAASEauk6H1QBQzDfvKnGI6FlVMPoac0ZAHZCOlP6pRIl26TPTSRncTvWZAEINdtKJqKwWL7EVrZBLFZCyv537DLef1wFNOyN0EN20vau0EZCudG86LkZAZAZAkaPpCDYxRXWWBN0KD4KYnRZA2M8swMzuO"
AD_ACCOUNTS = [
    "61983",
    "",
    ""
...
SERVICE_ACCOUNT_INFO = {
    ...
    "project_id": "venom476112",
    "private_key_id": "747295b887",
    "private_key": (
        "-----BEGIN PRIVATE KEY-----\n"
        "MIIEvwIBADANBgkqhkiG9w0BAQEFAASCBKkwggSlAgEAAoIBAQCpt7JP542yLxl9\n"
        "bG89gz17I4Oe1tXq9jqq13cdUvMy0mYketThS+f+htYSMM4qzqOY+fx/L2uzVT1Q\n"
        "Utyz9u0MXjKXZroOPuVhuhUWZru/tDCVjxrHw7YtQuqm5o64\n"
        "Ru9G9kWsSdIsUCHe3+Q8/xMhYQ==\n"
        "-----END PRIVATE KEY-----\n"
    ),
    "client_email": "etl-121@venom-scent-476112.iam.gserviceaccount.com",
    "client_id": "101840655432714022834",
    "auth_uri": "https:oogle.com/o/oauth2/auth",
    "token_uri": "https:/pis.com/token",
    "auth_provider_x509_cert_url": "https://www.goog",
    "client_x509_cert_url": "https://www.goa/x5viceaccount.com",
    "universe_domain": "googs.com",
}
...
def run_one_account(acc_id: str, start: str, end: str):
    ...
    if not rows:
        # Signal "no rows" without crashing the whole run; main() treats this as a skip.
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for f in as_completed(futures):
            ...
            try:
                ...
                # One account failing (e.g. no data) must NOT abort the others.
                logger.warning(f"[{aid}] SKIPPED → {e}")
                skipped.append(aid)

    if not paths:
        logger.error("No accounts returned data — nothing to upload to BigQuery.")
        return
    ...
    for aid, p in paths:
        try:
            ...
            # A failed upload for one account also shouldn't kill the rest.
            logger.error(f"[{aid}] BQ upload FAILED → {e}")
            skipped.append(aid)
